...
(function () {
    var c = !1, g = /xyz/.test(function () { xyz }) ? /\b_super\b/ : /.*/; this.Class = function () { };
    Class.extend = function (f) {
        function k() { !c && this.init && this.init.apply(this, arguments) } var r = this.prototype;
        c = !0; var q = new this; c = !1; for (var l in f) q[l] =
            "function" == typeof f[l] && "function" == typeof r[l] && g.test(f[l]) ? function (c, f) {
                return function ()
                { var g = this._super; this._super = r[c]; var a = f.apply(this, arguments); this._super = g; return a }
            }(l, f[l]) : f[l]; k.prototype = q; k.prototype.constructor = k; k.extend = arguments.callee;
        return k
    }
})();
(function (c, g) {
    function f(a) { return q ? q[k[a]] || a : a }
    var k = {
        "Add files": 0, "Start upload": 1, "Remove all": 2, Close: 3,
        "Select Files": 4, Preview: 5, "Remove file": 6, Bytes: 7, KB: 8, MB: 9, GB: 10, "Upload aborted": 11,
        "Upload all files": 12, "Seleccionar Archivos": 13, "Archivo cargado 100%": 14, "Max files number reached": 15,
        "Formato de archivo no válido": 16, "Tamaño de archivo no válido": 17
    }, r = {
        ...
        init: function (a, b, e, d, c) {
            ...
            this.uploadButton = c('<a title="' + f("Iniciar carga") + '" class="ax-upload ax-button" />').appendTo(this.buttons).append('<span class="glyphicon glyphicon-upload"></span>');
            this.removeButton = c('<a title="Eliminar archivo" class="ax-remove ax-button" />').appendTo(this.buttons).append('<span class="glyphicon glyphicon-trash"></span>');
            a.bootstrap && (this.li.addClass("media thumbnail label-info"),
            ...
            this.progressBar.addClass("bar"), this.buttons.css({ "border-top-left-radius": 0, "border-top-right-radius": 0 }),
            ...
            this.settings.editFilename && this.nameContainer.bind("dblclick", this, function (a) {
                ...
                a.stopPropagation();
                var b = c(this).children("input").val(); "undefined" != typeof b && (b = b.replace(/[|&;$%@"<>()+,]/g, "") + "." + a.data.ext,
                c(this).html(b),
                a.data.name = b,
                a.data.AU.hasAjaxUpload || a.data.xhr.children('input[name="ax-file-name"]').val(b))
            })
        }, doPreview: function () {
                    ...
                    e.css("cursor", "pointer").attr("src", d.target.result).click(function () {
                        ...
                        e.onload = function () {
                            var e = Math.min(c(window).width() / this.width, (c(window).height() - 100) / this.height),
                                ...
                                f = c("#ax-box").css({ top: f, height: e, width: h, left: g });
                            f.children("img").attr({ width: h, height: e, src: d.target.result });
                            c("#ax-box-fn").find("span").html(a + " (" + b.AU.formatSize(b.size) + ")");
                            f.fadeIn(500); c("#ax-box-shadow").css("height", c(document).height()).show()
                        };
                        e.src = d.target.result; c("#ax-box-shadow").css("z-index", 1E4); c("#ax-box").css("z-index", 10001)
                    })
                };
        ...
        askUser: function (a, b) {
            ...
            var e = c('<div class="ax-ask-inner"><div class="ax-ask-quest">' + b + "</div> </div>").appendTo(this.askDiv),
                d = c('<a title="Yes" class="ax-button ax-ask-yes"><span class="glyphicon glyphicon-upload"></span> <span>Yes</span></a>').appendTo(e),
            e = c('<a title="No" class="ax-button ax-ask-no"><span class="glyphicon glyphicon-trash"></span> <span>No</span></a>').appendTo(e);
            this.settings.bootstrap && (this.askDiv.addClass("alert"),
            d.addClass("btn btn-success btn-small").find(".ax-icon").addClass("icon-ok"),
            e.addClass("btn btn-danger btn-small").find(".ax-icon").addClass("icon-remove"));
            d.on("click", this, function (b) {
                a.call(b.data); b.data.askDiv.remove();
                b.data.askDiv = null
            });
        ...
        startUpload: function () {
            ...
            this.checkFileExists(function () {
                ...
                this.status = 2; this.AU.hasAjaxUpload ? this.uploadAjax() : this.AU.hasFlash ? this.AU.uploading || (this.AU.uploading = !0, this.AU.flashObj.uploadFile(this.pos)) : this.uploadStandard()
            ...
            var n = this;
            var timer = 0, perc = 0, timeTotal = 2500, timeCount = 1, cFlag;
            this.xhr.upload.addEventListener("abort", function (a) { n.AU.slots-- }, !1);
            ...
            this.xhr.upload.addEventListener("error", function (a) { n.onError(this.responseText) }, !1);
            function animateUpdate() {
                if (perc < timeTotal) {
                    perc += 50;
                    timer = setTimeout(animateUpdate, timeCount);
                    var x = (perc / timeTotal) * 100,
                        y = x.toFixed(1);
                    n.onProgress(y);
                }
            }
            this.xhr.onreadystatechange = function () {
                if (4 == this.readyState && 200 == this.status)
                    try {
                        var a = JSON.parse(this.responseText); 0 == e && (n.name = a.name, n.nameContainer.html(a.name));
            ...
            if (this.AU.useFormData) {
                ...
                d = -1 == a.url.indexOf("?") ? "?" : "&", this.xhr.open("POST", a.url + d + b.join("&"), a.async), this.xhr.setRequestHeader("Cache-Control", "no-cache"), this.xhr.setRequestHeader("X-Requested-With", "XMLHttpRequest"), this.xhr.setRequestHeader("Content-Type", "application/octet-stream"), this.xhr.send(m)
            ...
            c("#ax-main-frame").unbind("load").bind("load", this, function (a) {
                var b = null; this.contentDocument ? b = this.contentDocument : this.contentWindow && (b = this.contentWindow.document);
                try {
                    var e = c.parseJSON(b.body.innerHTML);
                    a.data.onProgress(100); a.data.onFinishUpload(e.name, e.size, e.status, e.info)
                }
                catch (d) {
            ...
            this.progressBar.css("width", 0);
            this.progressPer.html(f("Carga cancelada"))
        }, onError: function (a) {
            this.AU.slots--;
            this.currentByte = 0;
            this.status = -1;
            ...
            this.progressPer.html(f("Archivo cargado 100%"));
            this.settings.success.call(this, a); a = !0;
            for (b = 0; b < this.AU.files.length; b++) 1 != this.AU.files[b].status && -1 != this.AU.files[b].status && (a = !1);
            this.AU.upload_all && this.AU.uploadAll(); a && this.AU.finish(); this.settings.removeOnSuccess && this.AU.removeFile(this.pos)
            //console.log("carga finalizada");
        }, onProgress: function (a) {
            //console.log(a);
            this.progressBar.css("width", a + "%");
            if (a > 100)
                this.progressPer.html(100 + "%")
            else
                this.progressPer.html(a + "%")
        }
    }),
    t = function (a, b) {
        ...
        this.renderHtml();
        b.onInit.call(this); this.bindEvents()
    }; t.prototype = {
        preCheckSettings: function (a) {
            ...
            a.allowExt = c.map(a.allowExt, function (a, e) { return a.toLowerCase() });
            return a
        }, _init: function (a) {
            var b = this.settings; q = r[b.language]; a.addClass("ax-uploader").data("author", "http://www.albanx.com/"); 0 == c("#ax-main-frame").length && c('<iframe name="ax-main-frame" id="ax-main-frame" />').hide().appendTo("body"); 0 == c("#ax-box").length && c('<div id="ax-box"><div id="ax-box-fn"><span></span></div><img /><a id="ax-box-close" title="' + f("Close") + '"></a></div>').appendTo("body"); 0 == c("#ax-box-shadow").length && c('<div id="ax-box-shadow"/>').appendTo("body"); c("#ax-box-close, #ax-box-shadow").click(function (a) {
                a.preventDefault();
                c("#ax-box").fadeOut(500); c("#ax-box-shadow").hide()
            }); b.bootstrap && c("#ax-box-close").addClass("btn btn-danger").html('<span class="glyphicon glyphicon-trash icon-remove-sign"></span>'); for (b = "AX_" + Math.floor(100001 * Math.random()) ; 0 < c("#" + b).length;) b = "AX_" + Math.floor(100001 * Math.random()); a.attr("id", a.attr("id") ? a.attr("id") : b)
        }, checkUploadSupport: function () {
            var a = document.createElement("input"); a.type = "file"; this.hasAjaxUpload = "multiple" in a && "undefined" != typeof File && "undefined" != typeof (new XMLHttpRequest).upload;
            ...
            var a = this.settings; this.mainWrapper = c('<div class="ax-main-container" />').append('<h5 class="ax-main-title">' + f("Seleccionar archivos") + "</h5>").appendTo(this.container); this.max_size = a.maxFileSize; var b = a.maxFileSize.slice(-1); if (isNaN(b)) switch (this.max_size = this.max_size.replace(b, ""), b) {
                case "P": this.max_size *= 1024; case "T": this.max_size *= 1024; case "G": this.max_size *= 1024; case "M": this.max_size *= 1024; case "K": this.max_size *=
                1024
            } var e = "ax-browse-c ax-button", b = "ax-upload-all ax-button", d = "ax-clear ax-button", g = "glyphicon glyphicon-plus", h = "glyphicon glyphicon-upload", p = "glyphicon glyphicon-trash"; a.bootstrap && (e += " btn btn-primary", b += " btn btn-success", d += " btn btn-danger", g += " icon-plus-sign", h += " icon-play", p += " icon-remove-sign"); this.browse_c = c('<a class="' + e + '" title="' + f("Abre una carpeta para seleccionar archivos") + '" />').append('<span class="' + g + '"></span> <span class="ax-text">' + f("Agregar archivo...") + "</span>").appendTo(this.mainWrapper); this.browseFiles = c('<input type="file" class="ax-browse" name="ax_file_input" />').attr("multiple",
            ...
            this.uploadFiles = c('<a class="' + b + '" style="display:none;" " title="' + f("Inicia la carga de todos los archivos") + '" />').append('<span class="' + h + '"></span> <span class="ax-text">' + f("Iniciar carga de archivos") + "</span>").appendTo(this.mainWrapper);
            this.removeFiles = c('<a class="' + d + '" style="display:none;"title="' + f("Elimina de la vista previa todas los archivos") + '" />').append('<span class="' + p + '"></span> <span class="ax-text">' + f("Eliminar todo") + "</span>").appendTo(this.mainWrapper); this.fileList = c('<ul class="ax-file-list" />').appendTo(this.mainWrapper);
            a.bootstrap && this.fileList.addClass("media-list")
        }, bindEvents: function () {
            var a = this.settings; this.browseFiles.bind("change", this, function (a) { a = a.data; a.settings.enable && !a.hasFlash && (a.addFiles(a.hasAjaxUpload ? this.files : Array(this)), a.hasAjaxUpload ? this.value = "" : c(this).clone(!0).val("").appendTo(a.browse_c)) }); this.uploadFiles.bind("click", this, function (a) { a.data.settings.enable && a.data.uploadAll(); return !1 }); this.removeFiles.bind("click", this, function (a) {
                a.data.settings.enable && a.data.clearQueue();
                return !1
            }); 0 < c(a.form).length ? this.form = c(a.form) : "parent" == a.form && (this.form = this.container.parents("form:first")); if (null !== this.form && this.form !== g) { a.hideUploadForm && this.uploadFiles.hide(); var b = this.form.data("events"); null !== b && b !== g && null !== b.submit && b.submit !== g && (this.form_submit_event = b.submit); this.form.unbind("submit"); this.form.bind("submit.ax", this, function (a) { if (0 < a.data.files.length) return a.data.uploadAll(), !1 }) } if (this.hasAjaxUpload) {
                var b = "self" == a.dropArea ? this.container[0] :
                c(a.dropArea)[0], e = this; "self" == a.dropArea && this.mainWrapper.find(".ax-main-title").html(f("Seleccionar Archivos")); b.addEventListener("dragenter", function (a) { a.stopPropagation(); a.preventDefault() }, !1); b.addEventListener("dragover", function (b) { b.stopPropagation(); b.preventDefault(); e.settings.enable && (a.dropClass ? c(this).addClass(a.dropClass) : this.style.backgroundColor = a.dropColor) }, !1); b.addEventListener("dragleave", function (b) {
                    b.stopPropagation(); b.preventDefault(); e.settings.enable &&
                    (a.dropClass ? c(this).removeClass(a.dropClass) : this.style.backgroundColor = "")
                }, !1); b.addEventListener("drop", function (b) { e.settings.enable && (b.stopPropagation(), b.preventDefault(), e.addFiles(b.dataTransfer.files), this.style.backgroundColor = "", a.autoStart && e.uploadAll()) }, !1); c(document).unbind(".ax").bind("keyup.ax", function (a) { 27 == a.keyCode && c("#ax-box-shadow, #ax-box").fadeOut(500) })
            } this.enable(this.settings.enable)
        }, finish: function () {
            this.upload_all = !1; for (var a = [], b = 0; b < this.files.length; b++) a.push(this.files[b].name);
            this.settings.finish.call(this, a, this.files); this.settings.beforeSubmit.call(this, a, this.files, function () {
                if (null !== this.form && this.form !== g) {
                    for (var b = "function" == typeof this.settings.remotePath ? this.settings.remotePath() : this.settings.remotePath, c = 0; c < a.length; c++) this.form.append('<input name="ax-uploaded-files[]" type="hidden" value="' + (b + a[c]) + '" />'); this.form.unbind("submit.ax"); null !== this.form_submit_event && this.form_submit_event !== g && this.form.bind("submit", this.form_submit_event); b = this.form.find('[type="submit"]');
                    0 < b.length ? b.trigger("click") : this.form.submit()
                }
                this.uploadFiles.css("display", "none");
                this.removeFiles.css("display", "none");
            })
        }, addFiles: function (a) {
            for (var b = [], c = 0; c < a.length; c++) {
                var d, f, h; this.hasAjaxUpload || this.hasFlash ? (f = a[c].name, h = a[c].size) : (f = a[c].value.replace(/^.*\\/, ""), h = 0); d = f.split(".").pop().toLowerCase();


                var g = this.checkFile(f, h); "" == g ? (d = new l(a[c], f, h, d, this), this.files.push(d), b.push(d)) : this.settings.error.call(this, g, f)
            } this.settings.onSelect.call(this, b); this.settings.autoStart && this.uploadAll();
            this.uploadFiles.attr('style', function (i, style) {
                return style.replace(/display[^;]+;?/g, '');
            });
            this.removeFiles.attr('style', function (i, style) {
                return style.replace(/display[^;]+;?/g, '');
            });

        }, checkFile: function (a, b) {
            var e = a.split(".").pop().toLowerCase(),
            d = !!(this.files.length < this.settings.maxFiles), l = !!(0 <= c.inArray(e, this.settings.allowExt) || 0 == this.settings.allowExt.length), h = !!(b <= this.max_size), p = "function" === typeof this.settings.validateFile ? this.settings.validateFile.call(this, a, e, b) : "", k = ""; d || (k = k + f("Cantiad máximo de archivos excedido") + " : " + d + "\n"); l || (k = k + f("Extensión no válida") + " : " + e + "\n"); h || (k = k + f("Tamaño de archivo no válido") + " : " + b + "\n"); "" != p && p !== g && null !== p && (k += p); return k
            ...
            var d = this.settings, f = "function" == typeof d.remotePath ? d.remotePath() : d.remotePath, h = []; h.push("ax-file-path=" + (c ? encodeURIComponent(f) :
            ...
            d.thumbWidth); h.push("ax-random=" + 10001 * Math.random()); (this.settings.checkFileExists || this.settings.overrideFile) && h.push("ax-override=1"); a = "function" == typeof d.data ? d.data() : d.data; if ("object" == typeof a) for (var g in a) h.push(g + "=" + (c ? encodeURIComponent(a[g]) : a[g])); else if ("string" == typeof a && "" != a) for (c = a.split("&"), g = 0; g < c.length; g++) h.push(c[g]); return h
        ...
        init: function (a) { return this.each(function () { var b = c.extend({}, u, a), e = c(this).html(""), d = e.data("AU"); d !== g && null !== d || e.data("AU", new t(e, b)) }) }, clear: function () { return this.each(function () { c(this).data("AU").clearQueue() }) }, start: function () { return this.each(function () { c(this).data("AU").uploadAll() }) }, addFlash: function (a) { c(this).data("AU").addFiles(a) }, progressFlash: function (a, b) { c(this).data("AU").files[b].onProgress(a) },
    ...
    }; c.fn.ajaxupload = function (a, b) { if (s[a]) return s[a].apply(this, Array.prototype.slice.call(arguments, 1)); if ("object" !== typeof a && a) c.error("Method " + a + " does not exist on jQuery.AjaxUploader"); else return s.init.apply(this, arguments) }
})(jQuery);